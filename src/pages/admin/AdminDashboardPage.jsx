...
// ✅ Import หน้าตารางงานรถ (ที่เราสร้างไว้ที่หน้า user แต่เอามา reuse ใน admin ได้เลย)
import CarSchedulePage from '../schedule/CarSchedulePage' 
...
export default function AdminDashboardPage() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      // ✅ เพิ่มหน้าตารางงานรถ
      case 'schedule': return <CarSchedulePage />

      case 'analytics': return <div className="ad-table-card ad-empty">📊 ส่วนแสดงกราฟและสถิติเชิงลึก (Coming Soon)</div>
      case 'export': return <div className="ad-table-card ad-empty">📥 หน้าสำหรับ Export ข้อมูลเป็น Excel/CSV (Coming Soon)</div>
      case 'notification': return <div className="ad-table-card ad-empty">📢 ระบบส่งประกาศแจ้งเตือนหา User (Coming Soon)</div>
      case 'audit': return <div className="ad-table-card ad-empty">📝 บันทึกประวัติการกระทำของ Admin (Coming Soon)</div>
      case 'settings': return <div className="ad-table-card ad-empty">⚙️ ตั้งค่าระบบ/ราคา/เงื่อนไข (Coming Soon)</div>
  ...
  const getPageTitle = () => {
    const titles = {
      ...
      schedule: 'ตารางงานรถ (Schedule)', // ✅ เพิ่มชื่อหน้า
    ...
    <div className="ad-layout">
      <aside className="ad-sidebar">
        ...
        <nav className="ad-menu">
          ...
          {/* ✅ แก้ไขปุ่มนี้ให้ถูกต้อง */}
          <button className={`ad-menu-item ${activeTab === 'schedule' ? 'active' : ''}`} onClick={() => setActiveTab('schedule')}>🗓️ ตารางงานรถ</button>
